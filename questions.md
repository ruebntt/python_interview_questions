...
Последовательностью в Python перебор элементов с помощью индексов 
`__getitem__()` `__len__()`, `count()`, `index()`, `__contains__()` и `__reversed__()
list, tuple, range, str и bytes.
...
нерекурсивных копий - означает, что программа загружает только те файлы или данные, которые явно указаны в команде или запросе, без автоматического перехода по ссылкам для скачивания связанных элементов. 
